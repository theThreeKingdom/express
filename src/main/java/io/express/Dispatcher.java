...
import io.express.converter.ConverterFactory;
import io.express.persist.DBManager;
import io.express.renderer.JavaScriptRenderer;
import io.express.renderer.Renderer;
import io.express.renderer.TextRenderer;
import io.express.template.JspTemplateFactory;
...
class Dispatcher {
    ...
    void initAll(Config config) throws Exception {
        ...
        try {
            ...
            if (maxFileSize != null) {
                try {
                    long n = Evaluator.eval(maxFileSize);
                    if (n <= 0)
